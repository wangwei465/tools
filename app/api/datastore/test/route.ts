 ...
 * ES 走 `GET /` 取版本，Mongo 走 admin ping + buildInfo；两者都临时建连、用完即弃，
 * 不污染连接池。
 *
...
export async function POST(request: Request) {
  ...
  if (!body.uri?.trim()) {
    const label = body.type === "mongo" ? "连接串" : "服务地址";
  ...
  try {
    const version =
      conn.type === "mongo" ? await pingMongo(conn) : (await esPing(conn, TEST_TIMEOUT_MS)).version;
    return NextResponse.json({ ok: true, version, latencyMs: Date.now() - start });
  } catch (err) {
    return NextResponse.json({ ok: false, error: describeError(err) });
  }
}
