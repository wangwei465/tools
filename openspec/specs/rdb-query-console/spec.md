# rdb-query-console Specification

## Purpose

关系型数据源的 SQL 查询台——提供 SQL 编辑与执行，结果以表格/JSON 双视图呈现并保留结果集列顺序，对大整数、时间、二进制与 `NULL` 做保真呈现；对裸 `SELECT` 注入行数上限并回显实际执行的 SQL，不可改写时按硬上限截断并标注；接入既有安全闸门的只读拦截与危险操作二次确认，并为语句执行设置超时与可读提示。
