# api-request-history Specification

## Purpose

请求历史——每次经代理发送请求后向 `api_history` 追加一条含请求快照与状态/耗时/大小的记录（成功与错误均落历史，来自已保存节点则关联 `node_id`），提供按时间倒序的历史面板回看、把历史快照重放到 tab，以及历史持久化与单条删除/清空清理。
