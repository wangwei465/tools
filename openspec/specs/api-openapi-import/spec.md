# api-openapi-import Specification

## Purpose

OpenAPI / Swagger 文档导入——在「接口调试」中粘贴 Swagger 2.0 / OpenAPI 3.0 / 3.1 文档原文（JSON 或 YAML），解析归一化后生成集合树（按 tag → path 首段 → 未分类三级回退分组）、请求节点、环境与 `baseUrl` 变量；含安全方案到 Auth 的映射、文档内 `$ref` 解析与循环截断、示例请求体生成、导入前预览与数量上限、重复导入新建副本、单事务原子写入与降级项报告；解析逻辑为不依赖网络与数据库的纯函数。
