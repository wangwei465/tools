# rdb-catalog Specification

## Purpose

关系型数据源的结构目录——以「库 → schema → 表」三级模型浏览 MySQL 与 PostgreSQL 的库表结构（MySQL 折叠 schema 层，PostgreSQL 展示完整三级并绑定单库），查看表的列结构与索引信息，按层级懒加载并支持按表名过滤；目录查询失败时给出可读原因而非数据库原始堆栈，元数据行到目录模型的映射实现为可独立单元测试的纯函数。
