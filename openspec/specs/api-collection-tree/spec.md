# api-collection-tree Specification

## Purpose

侧边栏集合树——以 `api_nodes` 邻接表结构（`parent_id`）展示 `folder` 与 `request` 节点的层级，支持展开/折叠、新建、重命名、删除（文件夹级联删除子树）、拖拽移动与同级排序，并将所有增删改移持久化到 `api_nodes` 表。
