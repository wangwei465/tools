# text-toolkit Specification

## Purpose

文本工具面板——单页多标签的纯前端文本处理工具，涵盖行级处理（去重/排序/清理/加缀/行号）、两组文本的集合运算、命名风格转换、批量替换、表格格式互转（CSV/TSV⇄JSON⇄Markdown）与文本统计，全部客户端计算、不落库、不发网络请求；含工具导航挂载、与「编码转换」「SQL 工具」的功能边界指引，以及统一的输入/输出/复制/错误交互与处理规模上限。
