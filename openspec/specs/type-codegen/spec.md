# type-codegen Specification

## Purpose

类型代码生成——由一份 JSON 样本推断结构并生成 TypeScript interface、Java POJO、Go struct（带 json tag）与 JSON Schema 四种目标输出；覆盖数组字段并集与可选性推断、嵌套结构的具名子类型、不可推断值的保守取值与「需人工确认字段」清单、数值范围与精度处理、非法标识符转义与原键名映射保留，并设有嵌套深度上限；推断层与生成层分离，均为可独立测试的纯函数。
